...
class CurvatureInterface:
    ...
    def last_layer_jacobians(self, batch):
        ...
        f : torch.Tensor
            output function `(batch, outputs)`
        """
        f, phi = self.model.forward_with_features(**batch)
        bsize = phi.shape[0]
        output_size = int(f.numel() / bsize)

        # calculate Jacobians using the feature vector 'phi'
        identity = torch.eye(output_size, device=batch['input_ids'].device).unsqueeze(0).tile(bsize, 1, 1)
